...
def _ornek_veri_yukle() -> None:
    """Veritabanı boşsa örnek veri ekler; dolu ise atlar."""
    with get_conn() as conn:
        if conn.execute("SELECT COUNT(*) FROM sporcular").fetchone()[0] > 0:
            print("Örnek veri zaten mevcut, atlanıyor.")
            return

        # Kulüpler
        cur = conn.execute(
            """INSERT OR IGNORE INTO kulupler
               (ad, yetkili_adi, telefon, sezon)
               VALUES (?,?,?,?)""",
            ("Lefkoşa Bisiklet Kulübü", "Ahmet Yılmaz", "0392 000 0001", "2026")
        )
        kid1 = cur.lastrowid
        cur = conn.execute(
            """INSERT OR IGNORE INTO kulupler
               (ad, yetkili_adi, telefon, sezon)
               VALUES (?,?,?,?)""",
            ("Gazimağusa Bisiklet Spor", "Mehmet Demir", "0392 000 0002", "2026")
        )
        kid2 = cur.lastrowid

        # Sporcular
        cur = conn.execute(
            """INSERT OR IGNORE INTO sporcular
               (ad, soyad, kimlik_no, dogum_tarihi, telefon, spor_dairesi_kayitli)
               VALUES (?,?,?,?,?,?)""",
            ("Ali", "Kaya", "1234567890", "2000-05-15", "0548 111 1111", 1)
        )
        sid1 = cur.lastrowid or conn.execute(
            "SELECT id FROM sporcular WHERE kimlik_no='1234567890'"
        ).fetchone()[0]
        ...
        cur = conn.execute(
            """INSERT OR IGNORE INTO sporcular
               (ad, soyad, kimlik_no, dogum_tarihi, telefon, spor_dairesi_kayitli)
               VALUES (?,?,?,?,?,?)""",
            ("Zeynep", "Arslan", "9876543210", "2008-03-20", "0548 222 2222", 1)
        )
        sid2 = cur.lastrowid or conn.execute(
            "SELECT id FROM sporcular WHERE kimlik_no='9876543210'"
        ).fetchone()[0]

        conn.execute(
            """INSERT OR IGNORE INTO veli_bilgileri
               (sporcu_id, ad_soyad, telefon) VALUES (?,?,?)""",
            (sid2, "Fatma Arslan", "0548 333 3333")
        )

        # Lisanslar
        _spor_dairesi_kontrol(sid1, conn)
        cur = conn.execute(
            """INSERT INTO lisanslar
               (sporcu_id, kulup_id, lisans_turu, sezon,
                saglik_raporu, baska_federasyon_beyani, lisans_ucreti_odendi)
               VALUES (?,?,?,?,?,?,?)""",
            (sid1, kid1, "Ulusal", "2026", 1, 1, 1)
        )
        lid1 = cur.lastrowid

        _spor_dairesi_kontrol(sid2, conn)
        cur = conn.execute(
            """INSERT INTO lisanslar
               (sporcu_id, kulup_id, lisans_turu, sezon,
                saglik_raporu, veli_muvafakati,
                baska_federasyon_beyani, lisans_ucreti_odendi)
               VALUES (?,?,?,?,?,?,?,?)""",
            (sid2, kid1, "Ulusal", "2026", 1, 1, 1, 1)
        )

        # Yurt dışı yarış izni
        cur = conn.execute(
            """INSERT INTO yurtdisi_yaris_izinleri
               (sporcu_id, lisans_id, yaris_adi, ulke,
                kulup_organizasyon, yaris_tarihi, kulup_yazisi)
               VALUES (?,?,?,?,?,?,?)""",
            (sid1, lid1, "Tour de Türkiye 2026", "Türkiye",
             "Fenerbahçe Bisiklet", "2026-09-15", 1)
        )
        conn.execute(
            "UPDATE yurtdisi_yaris_izinleri SET durum='Onaylandı', "
            "federasyon_izin_tarihi=date('now') WHERE id=?",
            (cur.lastrowid,)
        )
...
if __name__ == "__main__":
    ...
    print("\n=== Aktif Lisanslar (2026) ===")
    for row in aktif_lisanslar("2026"):
        print(f"  {row['ad']} {row['soyad']} | {row['lisans_turu']} | {row['kulup_adi']}")

    print("\n=== Bekleyen Transferler ===")
    for row in bekleyen_transferler():
        print(f"  {row['ad']} {row['soyad']}: {row['eski_kulup']} → {row['yeni_kulup']}")

    print("\n=== Kulüp Sporcu Sayıları (2026) ===")
    for row in kulup_sporcu_sayilari("2026"):
        print(f"  {row['kulup']}: {row['sporcu_sayisi']} sporcu")
